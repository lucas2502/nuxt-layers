...
describe('Helper: lib/logic', () => {
  ...
  test('should isEmpty not accept empty string, null or undefined', () => {
    const res =
      Helper.isEmpty(test1) && Helper.isEmpty(test2) && Helper.isEmpty(test3);
  ...
  test('should isDefined not accept null or undefined', () => {
    const res = Helper.isDefined(test2) && Helper.isDefined(test3);
    expect(res).toBe(false);
  });

  test('should isNotDefined accept null or undefined', () => {
    const res = Helper.isNotDefined(test2) && Helper.isNotDefined(test3);
    expect(res).toBe(true);
  });
});
